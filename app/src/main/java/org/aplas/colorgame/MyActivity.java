...
public class MyActivity extends AppCompatActivity {
    ...
    private void initTimer() {
        ...
        long countDownInterval = getResources().getInteger(R.integer.mintimer) + 1;
        countDown = new CountDownTimer(millisInFuture, countDownInterval) {
            public void onTick(long millisUntilFinished) {
                timer.setText("" + String.format(FORMAT, TimeUnit.MILLISECONDS.toSeconds(millisUntilFinished) - TimeUnit.MINUTES.toSeconds(TimeUnit.MILLISECONDS.toMinutes(millisUntilFinished)),
    ...
    private void correctSubmit() {
        ...
        boolean maxScore = progress.getProgress() == getResources().getInteger(R.integer.maxScore);
        if (maxScore) {
            countDown.cancel();
